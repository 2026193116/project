...
class WordSpawner:
    ...
    def spawn(self, level: int, active_words: list) -> Word:
        ...
        # 만약 모든 단어가 화면에 다 나와 있다면 어쩔 수 없이 전체 풀에서 선택
        if not available_words:
            available_words = self.word_pool
            
        chosen_text = random.choice(available_words)

        # 레벨별 속도 증가 및 최대 속도 제한(WORD_SPEED_LIMIT) 적용
...
class GameState:
    def __init__(self):
        ...
        # 쿨타임 시스템을 위한 변수 추가
        self.last_shout_success_time = 0  # 마지막으로 소리를 질러 '성공적으로 밀어낸' 시간
        self.is_cooling_down = False      # 현재 3초 쿨타임 중인지 여부
        self.was_shouting_last_frame = False # 연속 소리 지르기 판정용
